...
class DataReader(object):
    ...
    def __init__(self, reload_from_original_data=False):
        ...
        self.DATASET_SPLIT_ROOT_FOLDER = os.path.join(
            os.path.split(os.path.dirname(__file__))[0], self.__DATASET_SPLIT_SUBFOLDER
        )
    ...
    def load_data(self, save_folder_path=None):
        ...
        if save_folder_path is None:
            save_folder_path = (
                self.DATASET_SPLIT_ROOT_FOLDER
                + self._get_dataset_name_root()
        ...
        if save_folder_path is not False and not self.reload_from_original_data:
            ...
            try:
                ...
                self._print(
                    "Reading split from {} caused the following exception...".format(
                        save_folder_path
                    )
                )
                traceback.print_exc()
                raise Exception(
                    "{}: Exception while reading split".format(self._get_dataset_name())
                )

        self._print("Loading original data2")
