...
class _MultDAE_original(object):
    def __init__(self, p_dims, q_dims=None, lam=0.01, lr=1e-3, random_seed=None):
        ...
        if q_dims is None:
            ...
            assert (
                q_dims[0] == p_dims[-1]
            ), "Input and output dimension must equal each other for autoencoders."
            assert (
                q_dims[-1] == p_dims[0]
    ...
    def build_graph(self):
        ...
        reg = keras.regularizers.l2(self.lam)
    ...
    def q_graph(self):
        ...
        for i, (w, b) in enumerate(zip(self.weights_q, self.biases_q)):
            ...
            if i != len(self.weights_q) - 1:
                ...
                KL = tf.reduce_mean(
                    input_tensor=tf.reduce_sum(
                        input_tensor=0.5 * (-logvar_q + tf.exp(logvar_q) + mu_q**2 - 1),
                        axis=1,
                    )
                )
...
class MultVAERecommender(
    BaseRecommender, Incremental_Training_Early_Stopping, BaseTempFolder
):
    ...
    def _compute_item_score(self, user_id_array, items_to_compute=None):
        ...
        item_scores_to_compute = self.sess.run(
            self.logits_var, feed_dict={self.vae.input_ph: URM_train_user_slice}
        )
    ...
    def fit(
        ...
        **earlystopping_kwargs
        ...
        try:
            ...
            self.load_model(
                self.temp_file_folder, file_name="_best_model", create_zip=False
            )
    ...
    def save_model(self, folder_path, file_name=None, create_zip=True):
        ...
        dataIO.save_data(
            file_name="fit_attributes", data_dict_to_save=data_dict_to_save
        )
        ...
        if create_zip:
            ...
            shutil.make_archive(
                ...
                base_dir=None,
            )  # start archiving from the root_dir
    ...
    def load_model(self, folder_path, file_name=None, create_zip=True):
        ...
        if create_zip:
            shutil.unpack_archive(
                folder_path + file_name + ".zip", folder_path + file_name + "/", "zip"
            )
...
class MultVAERecommender_OptimizerMask(MultVAERecommender):
    ...
    def fit(
        ...
        **earlystopping_kwargs
        ...
        super(MultVAERecommender_OptimizerMask, self).fit(
            epochs=epochs,
