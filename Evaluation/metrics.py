...
def average_precision(is_relevant, relevant_items=None):
    aux = (
        relevant_items.shape[0] if relevant_items is not None else is_relevant.shape[0]
    )
    ...
    if len(is_relevant) == 0:
        ...
        a_p = np.sum(p_at_k) / np.min([is_relevant.shape[0], aux])
...
class _Global_Item_Distribution_Counter(_Metrics_Object):
    ...
    def __init__(self, n_items, ignore_items):
        ...
        self.recommended_counter = np.zeros(n_items, dtype=np.float32)
        self.ignore_items = ignore_items.astype(np.int32).copy()
...
class Users_In_GT(_Metrics_Object):
    ...
    def __init__(self, URM_test, ignore_users):
        ...
        self.ignore_users = ignore_users.astype(np.int32).copy()
