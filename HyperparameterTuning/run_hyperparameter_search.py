...
def runHyperparameterSearch_Hybrid(
    ...
    if ICM_object_last_test is not None:
        ICM_object_last_test = ICM_object_last_test.copy()
    ...
    try:
        ...
        if recommender_class in [
            ...
            if URM_train_last_test is not None or ICM_object_last_test is not None:
                recommender_input_args_last_test = recommender_input_args.copy()
                if URM_train_last_test is not None:
                    recommender_input_args_last_test.CONSTRUCTOR_POSITIONAL_ARGS[0] = (
                        URM_train_last_test
                    )
                if ICM_object_last_test is not None:
                    recommender_input_args_last_test.CONSTRUCTOR_POSITIONAL_ARGS[1] = (
                        ICM_object_last_test
                    )
            else:
            ...
            hyperparameters_range_dictionary = {
                ...
                "num_threads": Categorical([multiprocessing.cpu_count()]),
            }
